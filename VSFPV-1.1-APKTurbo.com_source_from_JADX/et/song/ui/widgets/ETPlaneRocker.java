...
public class ETPlaneRocker extends View {
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                case 1:
                    ...
                    ETPlaneRocker.this.planeCoord[0] = (int) (q.f9x - ((float) ETPlaneRocker.this.centerCoord[0]));
                    ETPlaneRocker.this.planeCoord[1] = (int) (((float) ETPlaneRocker.this.centerCoord[1]) - q.f10y);
                    ETPlaneRocker.this.planeStatus = 2;
                    ETPlaneRocker.this.invalidate();
                    return;
                case 3:
                    ETPlaneRocker.this.planeCoord[0] = 0;
                    ETPlaneRocker.this.planeCoord[1] = 0;
                    ETPlaneRocker.this.ballCoord[0] = 0;
                    if (ETPlaneRocker.this.ismLock) {
                        ETPlaneRocker.this.ballCoord[1] = 0;
                    }
                    ETPlaneRocker.this.invalidate();
                    return;
                default:
    ...
    class C00992 implements Runnable {
        ...
        public void run() {
            if (ETPlaneRocker.this.mode != 1) {
                ETPlaneRocker.this.queue.clear();
            }
            if (ETPlaneRocker.this.queue.isEmpty()) {
                log.m1e("empty");
                ETPlaneRocker.this.planeStatus = 0;
                ETPlaneRocker.this.pointIdx = 0;
                Message msg = new Message();
                msg.what = 3;
                ETPlaneRocker.this.mHandler.sendMessage(msg);
                return;
            }
            trackPoint q = (trackPoint) ETPlaneRocker.this.queue.poll();
            if (ETPlaneRocker.this.pointIdx == 0) {
                ETPlaneRocker.this.PointNoOne = q;
            } else if (ETPlaneRocker.this.pointIdx == 1) {
                ETPlaneRocker.this.PointNoFive = q;
                float xie = (float) Math.sqrt(Math.pow((double) (ETPlaneRocker.this.PointNoFive.f9x - ETPlaneRocker.this.PointNoOne.f9x), 2.0d) + Math.pow((double) (ETPlaneRocker.this.PointNoFive.f10y - ETPlaneRocker.this.PointNoOne.f10y), 2.0d));
                float sinAngle = (ETPlaneRocker.this.PointNoOne.f10y - ETPlaneRocker.this.PointNoFive.f10y) / xie;
                ETPlaneRocker.this.ballCoord[0] = (int) (((ETPlaneRocker.this.PointNoFive.f9x - ETPlaneRocker.this.PointNoOne.f9x) / xie) * 44.0f);
                ETPlaneRocker.this.ballCoord[1] = (int) (sinAngle * 44.0f);
            }
            ETPlaneRocker eTPlaneRocker = ETPlaneRocker.this;
            int access$8 = eTPlaneRocker.pointIdx + 1;
            eTPlaneRocker.pointIdx = access$8;
            if (access$8 > 1) {
                ETPlaneRocker.this.pointIdx = 0;
            }
            msg = new Message();
            ...
            msg.what = 1;
            ETPlaneRocker.this.mHandler.sendMessage(msg);
            ETPlaneRocker.this.mHandler.postDelayed(ETPlaneRocker.this.updatePlanePos, 40);
        }
    }
    ...
    public ETPlaneRocker(Context context) {
        super(context);
        ...
        this.mHandler = new C00981();
        this.updatePlanePos = new C00992();
        this.mContext = context;
        initView();
    }

    public ETPlaneRocker(Context context, AttributeSet attrs) {
        super(context, attrs);
        this.centerCoord = new int[2];
        this.ballCoord = new int[2];
        this.planeCoord = new int[2];
        ...
        if (event.getAction() == 0) {
            ...
            if (this.mode == 1) {
                ...
                if (this.pointLength >= 10) {
                    ...
                    for (int i = 0; i < chu; i++) {
                        ETPlaneRocker eTPlaneRocker = this;
                        trackPoint p = new trackPoint();
                        p.f9x = ((float) ((int) this.startX)) + ((10.0f * cosAngle) * ((float) i));
                        p.f10y = ((float) ((int) this.startY)) + ((10.0f * sinAngle) * ((float) i));
                        try {
