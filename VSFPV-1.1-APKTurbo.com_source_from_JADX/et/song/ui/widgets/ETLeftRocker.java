...
public class ETLeftRocker extends View {
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                case 1:
                    ...
                    ETLeftRocker.this.planeCoord[0] = (int) (q.f7x - ((float) ETLeftRocker.this.centerCoord[0]));
                    ETLeftRocker.this.planeCoord[1] = (int) (((float) ETLeftRocker.this.centerCoord[1]) - q.f8y);
    ...
    class C00972 implements Runnable {
        ...
        public void run() {
            ...
            if (ETLeftRocker.this.queue.isEmpty()) {
                ...
                msg.what = 3;
                ETLeftRocker.this.mHandler.sendMessage(msg);
                return;
            }
            trackPoint q = (trackPoint) ETLeftRocker.this.queue.poll();
            if (ETLeftRocker.this.pointIdx == 0) {
                ETLeftRocker.this.PointNoOne = q;
            } else if (ETLeftRocker.this.pointIdx == 1) {
                ETLeftRocker.this.PointNoFive = q;
                float xie = (float) Math.sqrt(Math.pow((double) (ETLeftRocker.this.PointNoFive.f7x - ETLeftRocker.this.PointNoOne.f7x), 2.0d) + Math.pow((double) (ETLeftRocker.this.PointNoFive.f8y - ETLeftRocker.this.PointNoOne.f8y), 2.0d));
                float sinAngle = (ETLeftRocker.this.PointNoOne.f8y - ETLeftRocker.this.PointNoFive.f8y) / xie;
                ETLeftRocker.this.ballCoord[0] = (int) (((ETLeftRocker.this.PointNoFive.f7x - ETLeftRocker.this.PointNoOne.f7x) / xie) * 44.0f);
                ETLeftRocker.this.ballCoord[1] = (int) (sinAngle * 44.0f);
            }
            ETLeftRocker eTLeftRocker = ETLeftRocker.this;
            int access$8 = eTLeftRocker.pointIdx + 1;
            eTLeftRocker.pointIdx = access$8;
            if (access$8 > 1) {
                ETLeftRocker.this.pointIdx = 0;
            }
            msg = new Message();
            ...
            msg.what = 1;
            ETLeftRocker.this.mHandler.sendMessage(msg);
            ETLeftRocker.this.mHandler.postDelayed(ETLeftRocker.this.updatePlanePos, 40);
        }
    }
    ...
    public ETLeftRocker(Context context) {
        ...
        this.mHandler = new C00961();
        this.updatePlanePos = new C00972();
        this.mContext = context;
        initView();
    }

    public ETLeftRocker(Context context, AttributeSet attrs) {
        super(context, attrs);
        this.centerCoord = new int[2];
        this.ballCoord = new int[2];
        this.planeCoord = new int[2];
        ...
        if (event.getAction() == 0) {
            ...
            if (this.mode == 1) {
                ...
                if (this.pointLength >= 10) {
                    ...
                    for (int i = 0; i < chu; i++) {
                        ETLeftRocker eTLeftRocker = this;
                        trackPoint p = new trackPoint();
                        p.f7x = ((float) ((int) this.startX)) + ((10.0f * cosAngle) * ((float) i));
                        p.f8y = ((float) ((int) this.startY)) + ((10.0f * sinAngle) * ((float) i));
                        try {
